...
inline void ReplaceAll(std::string* base_string, std::string_view old_string,
                       std::string_view new_string) {
  auto pos = base_string->find(old_string);
  while (pos != std::string::npos) {
    base_string->replace(pos, old_string.length(), new_string);
    pos = base_string->find(old_string, pos + new_string.length());
  }
}

inline bool Search(std::string_view target, std::string_view base_string) {
  if (target.empty() || base_string.empty()) return false;
